...
public class Sorting {
    ...
    public static <T> void mergeSort(T[] arr, Comparator<T> comparator) {
        ...
        if (arr == null || comparator == null) {
            throw new IllegalArgumentException("The array or comparator you entered is null");
        }
        if (arr.length <= 1) {
            return ;
        }
    ...
    private static <T> T[] mergeSortHelper(T[] arr, Comparator<T> comparator) {
        ...
        arr = merge(left, right, comparator);
        return arr;
    }
    ...
    private static <T> T[] merge(T[] left , T[] right , Comparator<T> comparator) {
        ...
        while (leftPointer < left.length || rightPointer < right.length) {
            if (rightPointer < right.length && leftPointer < left.length) {
                if (comparator.compare(left[leftPointer] , right[rightPointer]) <= 0) {
                    // Replacing and incrementing the pointers
