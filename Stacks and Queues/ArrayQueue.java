...
public class ArrayQueue<T> {
    ...
    public void enqueue(T data) {
        if (data == null) {
            throw new IllegalArgumentException("The data entered is null! It cannot be added to the queue.");
        }
        if (size == backingArray.length) {
            Object[] newArray = new Object[backingArray.length * 2];
            for (int i = 0; i < backingArray.length; i++) {
                newArray[i] = backingArray[(front + i) % size];
            }
    ...
    public T dequeue() {
        ...
        Object removed = backingArray[front];
